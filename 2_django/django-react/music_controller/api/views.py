...
class UserInRoom(APIView):
    def get(self, request, format=None):
        ...
        # Takes python dict and serialize into JSON format to be sent as response
...
class UpdateRoom(APIView):
    ...
    def patch(self, request, format=None):  # patch means update
        if not self.request.session.exists(self.request.session.session_key):
            self.request.session.create()
        ...
        if serializer.is_valid():
            ...
            if room.host != user_id:  # check if user is host
                return Response({"msg": "You are not the host of room"}, status=status.HTTP_403_FORBIDDEN)
